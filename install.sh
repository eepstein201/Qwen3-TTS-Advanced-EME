...
print_summary() {
    ...
    echo "     ${CYAN}tts server start${NC}"
    echo ""
    echo "  2. Generate speech:"
    echo "     ${CYAN}tts \"Hello, world!\" -o hello${NC}"
    echo ""
    echo "  3. Launch web interface:"
    echo "     ${CYAN}tts ui${NC}"
    echo ""
    echo "  4. Stop the server when done:"
    echo "     ${CYAN}tts server stop${NC}"
    echo ""
    echo -e "${BOLD}Commands:${NC}"
    echo "  ${CYAN}tts${NC}                - Generate speech from text"
    echo "  ${CYAN}tts server start${NC}   - Start the persistent TTS server"
    echo "  ${CYAN}tts server stop${NC}    - Stop the TTS server"
    echo "  ${CYAN}tts voice create${NC}   - Create a voice clone from audio"
    echo "  ${CYAN}tts ui${NC}             - Launch web interface"
    echo "  ${CYAN}tts config${NC}         - Reconfigure settings (model size, quantization)"
    echo ""
