...
- **Status: DONE (branch `fix/on-demand-model-load-generate`, 2026-09-13; worktree off `230a3c81`; evidence doc `docs/testing/step2-on-demand-load-2026-09-13.tdd.md`).** The hard gate above was satisfied 2026-09-13 by 4B (PR #297): ≥5-cycle load/unload campaign, no death, no leak, flat 8bit footprint, warm reloads 0.7–2 s. Implementation: both `handle_generate` and `handle_generate_stream` route an empty model slot through `model_loading.load_model_deduped` — `/load-model`'s per-load-record owner (claim/attach dedup, #192 leaf-locked warm-up, PRF-5 recovery) — at the exact point the old 503 fired: after validation + the memory guard, before the pre-lock cache check, and before the handler takes `inference_lock` (the warm-up's leaf acquisition preserves the inference_lock-outermost order; `MODEL_LOAD_LOCK` is never held across an await). Load failure surfaces `/load-model`'s exact sanitized classified shape (owner failures are 500 `load_failed`, `recovery: restart` — the plan's "sanitized 503 shape" wording resolved in favor of exact `/load-model` parity); the residual `model_not_loaded` 503 (slot emptied again post-load) now points at `POST /load-model` / the Manage Models tab (`recovery: "load_model"`), not "restart". RED→GREEN: 7 new tests in `tests/test_generate_on_demand_load.py` (registered batch 3, "Server Infrastructure"), all RED pre-fix, all GREEN post-fix; the concurrency additions — two simultaneous first-requests load exactly once (claim/attach through the `/generate` path) and an on-demand load racing another mode's in-flight generation proves the no-deadlock interleaving by ordered markers (load completes while the sibling holds `inference_lock`; warm-up waits as a leaf; both 200) — the strongest race constructible without a live GPU, covering 4B's serial-only gap. Four legacy tests that pinned the 503-without-load updated (they otherwise attempted REAL multi-GB loads in-process); `tests/test_ai_regression.py`'s three live-server `model_not_loaded` e2e tests flagged for post-merge rework (e2e is dispatcher-owned). Gates: 18 passed `-k generate`; full non-E2E suite 3420 passed / 6 skipped / 0 failed; ruff clean; mypy clean; test_claude_md 4 passed (203 ≤ 300). Docs: CLAUDE.md API-table + Key-Settings + architecture rows updated in place; RUNBOOK's "on demand when a request needs them" wording is now actually true (no edit needed). Batch/E2E runner gates left to the dispatcher per lane rules.
...
### Step 3C — 3c: timeouts & MLX prompt naming bundle (EXECUTED 2026-09-11)

- **Status: DONE (branch `fix/phase3-timeouts-prompt-naming`, 2026-09-11; executed by parallel dispatch, RED→GREEN per item, gates re-verified by the dispatcher).** Item outcomes: (1) FIXED — `ASR_LOAD_TIMEOUT_SEC=900` added to `http_client.py` beside its five sibling constants and wired into the UI `/load-asr` call (`model_management.py`, drifted from the plan's :193 to :209-214); no server-side lockstep constant exists for `/load-asr` (it has no dedup-wait, unlike `/load-model`) so the UNLOAD-style constant+lockstep+source-grep guard is the full applicable pattern (`TestLoadAsrClientTimeout`, mirroring `TestUnloadAsrClientTimeout`). (2) FIXED — voice preview's flat `timeout=60` replaced with the shared `_generation_timeout(len(texts[0]))` (imported from its definition site, precedent `generate_server.py:203`); RED `60 != 600`. (3) PARTIAL — the three `cli_voice.py` `.pt` sites are FALSE POSITIVES with evidence: `:153` strips all three suffixes, and `:213/:242` are rebuild-scoped `.pt` REGENERATION behind `_ensure_torch_backend_for_rebuild()` (rebuild is torch-only by design); the real defect was `tools/create_voice.py:100-102` forcing `.pt` in MLX-only mode (whose Done hint then advertised a `.pt` that mode never writes) — FIXED via `strip_extension`, torch branch byte-identical (42 passed incl. torch tests). (4) FIXED — REPL `/prompt` is backend-aware (`.wav` addressing on MLX via the backend-aware `voice_prompt_exists`, bare `/prompt` lists via `list_voice_prompts()`); 4-test pin discovered a second patch seam (`generate_helpers.get_backend` module-load binding) corrected in the fix commit. Gates: 126 passed across 4 touched modules, batches 2/3/4 each 1/1, ruff clean, mypy no NEW errors (2 pre-existing in `create_voice.py` proven against base), attribution clean.
- **Findings carried (pre-existing, deliberately not touched):** (a) `generate_interactive.py:159` `preview_voice_prompt` does bare `resp.json()["results"][0]` — the exact pattern CLAUDE.md bans (`cancelled: true` batches can return fewer/empty `results`); candidate add-on for Step 3E (CLI/tools bundle) or a standing entry. (b) `/prompt` with a traversal-shaped arg raises `ValueError` from `safe_path_join` and escapes the REPL's command loop (pre-existing behavior class, unchanged). (c) Pre-existing mypy errors in `tools/create_voice.py` (`_resolve_transcript` str/None reassignment) — needs an owning entry.
- **Shared-paperwork deferral:** the executed-list line, `Open: 33 → 32`, and the master-plan M6 register mark land in the 3D branch's reconciliation commit (parallel-lane serialization — this branch deliberately edits only its own step block).
...
### Step 3D — 3e remainder: server hygiene bundle (minus M8, done in Step 1B) (EXECUTED 2026-09-11)

- **Status: DONE (branch `fix/phase3-server-hygiene`, 2026-09-11; parallel dispatch off `6188e5bb`, per-item RED→GREEN, gates re-verified by the dispatcher).** Items: (1) FIXED — `/stats` + `/models` now `asyncio.to_thread`-offloaded (`app.py:678,688`; sync handlers lazily import the engine + probe memory); source-inspection pins (`TestStatsModelsOffload`, the conftest-client-independent convention). (2) FIXED — streaming `pending_requests` entry now removed in a `finally` wrapping the WHOLE lock wait (`app_generation.py:806-814`, idempotent removal), closing the cancel/disconnect-while-queued leak; the `async with inference_lock:` shape was deliberately PRESERVED after a manual-acquire attempt broke the Step 0B AST pins (reverted; real change is 16 lines by `diff -w`); pinned by `TestPendingQueueCleanupOnAbandonedWait` (drives the generator's body_iterator directly, no TestClient receive trap). (3) FIXED remainder — decode/staging was already offloaded at `app_models.py:550,557`, `app_prompts.py:440,447`, and cache-hit reads; the still-inline sites are now offloaded via new helpers `_b64_encode`/`_stage_cache_tempfile` (b64 ENCODE of results, cache tempfile create+chmod, audio/wav content-negotiation DECODE). (4) FIXED — two layers: engine special-case (`inference.py:74-83`, actionable ValueError naming both knobs) + server-boundary check (`validation.py:379-409`, first check in `_validate_generation_request`, 400 with the fix; `load_config` is mtime-cached so no per-request I/O); other unknown backends keep the generic error. (5) FALSE POSITIVE — all three `FileNotFoundError` sinks already route through `_sanitize_error` at the exact sink (`app_generation.py:516-526,907-916`, `websocket.py:402-410`), landed in `649de486` (PR #281/Step 0F); plan line numbers had drifted. (6) CONFIRMED superseded by 0C (PR #270): `generation_state_guard.py` exists, every streaming touch routes through the guard, raw access pinned. (7) CONFIRMED superseded by 0A (PR #263): `MAX_CHUNK_CHARS_LIMIT` Field bounds at `validation.py:22,52`.
- **Gates:** 82 passed across the 5 touched modules; batch 3 1/1; batch 5's `test_ai_regression` (e2e-marked, live :5123 server running MAIN's code — module untouched by this diff) timed out under parallel-conda load twice and passed on direct idle run (`8 passed, 1 skipped`) — environmental, consistent with the known e2e-timeout class; ruff/mypy/bandit clean; attribution clean (tight grep over commit messages and added lines: zero matches on the banned authorship patterns).
- **Finding carried:** `_maybe_start_vllm_adapter` (`app_lifespan.py:324-337`) — docstring says "when backend is vllm" but it gates ONLY on `vllm.enabled`: vLLM is used whenever `vllm.enabled` regardless of `advanced.backend`, and `advanced.backend="vllm"` can never torch-fallback through the engine (item 4's error now says this explicitly). Doc/design cleanup candidate.
- **Shared-paperwork reconciliation (this branch carries 3C's deferred marks too):** executed list gains 3C (PR #292) + 3D; `Open: 33 → 31`; master-plan M1/M4/M5/M6 register rows + Phase-3 3c/3e item lines marked (LOW-paragraph marks deferred to 3E's paperwork — single writer for that blob).
...
### Step 3E — 3g: CLI/tools bundle (EXECUTED 2026-09-11/12)

- **Status: DONE (branch `fix/phase3-cli-tools-bundle`; executed across two sessions — the 2026-09-11 run was interrupted mid-item-6 with items 1–5 + item 6's test committed, resumed 2026-09-12 in the same worktree; RED→GREEN per item, all gates re-verified).** Item outcomes: (1) FIXED — `cli_voice.py` create now propagates `create_voice.main()`'s int return via `sys.exit` (`tts voice create` exits non-zero on failure); pin `TestVoiceCreate` (test_cli_ext.py). (2) FIXED — `uninstall_config` resets through the atomic `save_config` (temp + `os.replace`) instead of a truncate-in-place `open()`; pin `test_reset_writes_via_atomic_save_config` asserting `save_config` called once. (3) FIXED — bare `tts config` handles a missing wizard (clear stderr + exit 1) and `subprocess.TimeoutExpired` (clear stderr + exit 1, "no changes were made"); pin `TestConfigWizard` (missing/timeout, `result.exception is None`). (4) FIXED — healthcheck `size_str` loop gained `"TB"` (≥1 TiB previously left `size_str` unbound → `UnboundLocalError`); pin `test_terabyte_cache_formats_as_tb`. Originally ABSORBED-BY-7B, but the fix is a one-token loop extension, so it landed here; 7B's migration to `cli_output.fmt_bytes` remains free to subsume it. (5) FIXED — all three L20b pieces: cache-prune epilog `--unused 30d` → `30` (pinned that the advertised value parses as the option's `int` type, `TestPruneEpilogExample`), duplicate Size column dropped from `tts cache list` (row printed `size_formatted` into both `Size` and `Size on Disk`; `TestListTableSingleSizeColumn`), uninstall epilog flag-style `tts uninstall --models` examples rewritten to the shipped subcommand surface (`TestUninstallEpilogExamples`). Like (4), originally ABSORBED-BY-7B; these are user-visible wrong-command fixes, not the idiom migration 7B owns. (6) FIXED, two parts — the reference-audio load (sf.read/pydub can block forever on an unresponsive mount or FIFO) moved into a bounded daemon worker `_load_reference_audio_bounded` (60 s → `TimeoutError`; `TestLoadReferenceAudioBounded`, RED `AttributeError: no attribute '_AUDIO_LOAD_TIMEOUT_SEC'`), and the open/xdg-open preview-player calls (which already carried `timeout=10` but no handler) now catch `TimeoutExpired` in both `create_voice.py` and `generate_helpers.open_file` so a hung player cannot turn a successful create/generation into a non-zero exit (`test_torch_test_gen_player_timeout_does_not_fail_create`, `TestOpenFile::test_open_timeout_does_not_raise`). (7) FIXED — `_SUBMODULES` omitted `components` (8 entries vs 9 on-disk modules): fresh-interpreter `qwen3_tts.interface.ui.components` hit the PEP 562 `__getattr__` fallback and raised `AttributeError` (absolute imports worked, masking it); added, plus a drift guard pinning `_SUBMODULES` == on-disk module list (`TestUiPackageSubmoduleMap`, subprocess pin RED on the exact `AttributeError`). (8) FIXED — the interactive-mode output-filename prompt hardcoded "saved to ~/Downloads/" while the save honored `config["output_directory"]`; the prompt now advertises the real directory (`test_output_filename_prompt_shows_configured_dir`). (9) ADD-ON (from 3C's findings carried) FIXED — `preview_voice_prompt`'s bare `resp.json()["results"][0]` routed through the sanctioned `_first_result` guard, printing the `GenerationError` reason (`technical_detail`) and returning False (pins: `test_cancelled_empty_results_returns_false` RED on the literal `IndexError`, `test_empty_results_without_cancel_returns_false`); the same function's unconditional `.pt` append — the 3C-item-3 defect class, missed there — is now backend-aware (`.wav+.txt` on MLX), making `tts voice preview` work on the default MLX backend (`test_mlx_name_resolves_to_wav_pair`; `test_auto_appends_pt_extension` now pins torch explicitly for determinism).
- **Findings carried (pre-existing, deliberately not fixed here):** (a) torchless CI-proxy env (`.venv-310`, no librosa) fails 8 tests in the touched sweep + 9 in `test_create_voice_functions.py` — all `ensure_min_sample_rate` raising on 16 kHz fixtures needing a resample; counts identical on base `b83b85d8` (proven by re-running the identical selections at base), so CI either has librosa or is equally red — needs an owning entry (fixture rate bump or env extras). (b) `test_create_voice_functions.py` torch-path tests fail 3/6 when the class runs ALONE (module-order-dependent resampler state; full-module runs pass) — test-isolation quirk. (c) `tools/uninstall.py` `main()` remains a working legacy flag-style argparse interface (only its epilog EXAMPLES were wrong); the dual CLI surface stays. (d) `TTSError.format_cli()` still has no in-repo callers. (e) `GenerationError`'s `str()` is the generic banner "Audio generation failed." — the specific reason rides on `technical_detail`; future CLI handlers must print that or they all look identical.
- **Shared-paperwork deferral:** the executed-list line, `Open: 33 → 32`, and Phase-3 counter reconciliation land in the 3D branch's reconciliation commit (parallel-lane serialization — this branch edits only its own step block and the master-plan 3g row + LOW-blob annotations assigned to it).
...
- **Status: DONE (branch `fix/e2e-model-load-assertions`, 2026-09-12; live baseline before any edit, three post-edit live runs + a direct DOM probe; batch-6 module gate dispatcher-owned).** Item outcomes: (1) DONE — all five discarded `_wait_for_model_state()` returns are now `assertTrue(...)` with messages naming model + expected state ("design model did not report loaded=True/False on /health within 60s/30s of the UI Load/Unload click"); all five PASSED in every post-edit run — the server-side load/unload path itself is healthy. (2) DONE, narrowed — the five blanket `except Exception: pass` wrappers around the `wait_for_any_textarea_contains` waits now catch ONLY `playwright.sync_api.TimeoutError` (aliased `PlaywrightTimeoutError` in the module-top playwright try-block; the class is `skipUnless(HAS_PLAYWRIGHT)`-gated): a lagging status textarea is cosmetic because the `/health` assert is the authoritative gate, while any other exception (dead page, broken wait) propagates. The per-poll `try/except` inside `_wait_for_model_state`/`_ensure_model_unloaded` and every unrelated swallow-all are untouched. (3) DEVIATION (recorded, evidence-backed) — the three `wait_for_table_row_refreshed` checks were hardened to asserts per plan, then failed IDENTICALLY in two consecutive live runs (run 1 and a freshly-restarted run 2): server-confirmed design load, table never renders "Loaded" despite the helper's two Refresh retries — meeting the plan's ≥2-consecutive-runs criterion. Ground truth via a live DOM probe (chrome-devtools against the same UI build, gradio 6.20.0): the Manage Models table renders exactly ONE stale row (`design | Not loaded | 2500MB | Yes`) that never updates across a server-side unload→load cycle (the memory cell stays "2500MB" even while unloaded, where a fresh fetch renders "—"), while `/models` and `/health` both report design loaded=true and the 5s status-Timer self-heal ticks (`queue/join` → `queue/data`) flow continuously; clone/custom/asr rows are missing from the rendered DOM entirely. I.e. the I4 "dropped Dataframe re-render" quirk is PERSISTENT in gradio 6.20.0, not intermittent. The three table checks therefore reverted to warn-and-continue BY RECORDED DEVIATION (warn text now cites `/health` as the verification source; the helper's docstring carries the evidence pointer), the five server-side asserts stay hard, and the frozen-Dataframe defect is Step 4B input (`docs/testing/step4a-e2e-unhollow-2026-09-12.tdd.md`). A second, intermittent failure mode surfaced in run 2 only: test_10 died on `Locator.click: Timeout 30000ms` waiting for the Refresh button inside `wait_for_table_row_refreshed` (no visible tabpanel ≈ page/panel degraded ~30s after load; did not recur in runs 1/3) — recorded for 4B, deliberately NOT coded around (a dead page must fail loudly). Live evidence (per test, before → after): baseline with file unchanged = 08/09/10 all PASSED (hollow, 211.62s); run 1 post-edit = 08 FAILED (table assert) / 09 PASSED / 10 FAILED (table assert); run 2 (fresh restart, `/generation-status` idle-verified) = 08 FAILED (same) / 09 PASSED / 10 FAILED (Refresh-locator timeout); run 3 (deviation form, fresh restart, idle-verified) = 08/09/10 all PASSED (211.30s) with all five server asserts live — pass/fail is now trustworthy. Gates: `ruff check tests/test_e2e_playwright.py` clean (exit 0); batch 6 not run in this lane per coordinator directive (dispatcher-owned) — the lane's own accidentally-started instance was killed, its orphaned UI subprocesses cleaned, and the server left idle and healthy for the dispatcher's run.
...
- **Status: DONE (branch `fix/e2e-load-cycle-investigation`, 2026-09-13; docs-only lane — verdict (c), hypothesis root-caused as a misattribution, two tracked-issue drafts banked; evidence doc `docs/testing/step4b-load-cycle-investigation-2026-09-13.tdd.md`).** Live campaign on the PM2 server (main @ `0617c2b8` == worktree base, all-three-resident per the uncommitted main config, mlx 8bit): 5 full load→unload cycles each on design and custom (initial unload first, else cycle 1's "load" would be a no-op attach) + 3 unload→reload cycles on clone + residency restore = 13 measured loads / 16 unloads. **No death** (PM2 `restart_time` 27 before/after — the only bump was the pre-campaign CLI restart; PID unchanged; campaign log segment 57 lines, zero warnings/tracebacks; script exit 0). **No leak** (post-unload footprint flat ±30 MB: design 9507→9485, custom 9458→9487, clone 9461→9464 MB; `mlx_memory_active_mb` bit-identical per residency every cycle — 6017.45 / 3020.25 / 0.0; load latencies IMPROVE, design 4.7→0.7 s = MLX buffer-cache reuse). Metric note: `ps rss` undercounts Metal/IOAccelerator memory ~600× (15.7 vs 9,549 MB) — `footprint <pid>` used instead. **Historical "log ends mid-load" deaths root-caused** (scanner over both rotations, 9 signatures): mechanism B = the Sep-5 15:52 event was an operator restart storm during the PM2 6.0.14→7.0.4 upgrade (every exit preceded by "Stopping app", zero unsolicited crashes; a stop landed 5 s into a startup load; graceful "shutting down…" logged, 15-min server-less gap; kernel unified log for the window has ZERO kill/memorystatus events — definitively not memory); mechanism A = the May-18/Jul-26/Aug-1 hard kills all sit on the `/update-model-config` unload-all→reload-all path (Jul 26 racing an active cancelled generation; Aug 1 a second design load starting during the first's warm-up = the pre-#214 double-allocation class, since fixed; 420 historical "Low memory" warnings all cluster on this same config-churn regime incl. bf16, where 3×bf16 ≈ 21 GB > 16 GB). Distractor ruled out: the April PM2 crashloop (41,692 process starts) is the pre-startup-lock `address already in use` double-start bug. Disposition: no product fix warranted (the pure load/unload path is healthy; the leak hypothesis is DISCONFIRMED at 8bit ≥5-cycle scope); two tracked-issue drafts ride the evidence doc — finding-8 (frozen Manage Models Dataframe, carried from 4A; NO connection found between server memory state and the UI defect) and finding-9 (`unload_model_cleanup()` MLX branch lacks `mx.clear_cache()` — unloaded weights stay in MLX's in-process Metal cache: 9.46 GB footprint with 0 models loaded, `mlx_active` 0.0; retained-but-purgeable/reused = NOT a leak, but "unload to free memory" doesn't promptly return memory to the OS). Deviations recorded in the evidence doc (initial unload, footprint-not-ps-rss, kernel-log retention limit for pre-Sep-5 windows, no :5125 second instance since no fix to verify).
- **Also queued here (2026-09-13, from the 4A DOM trace + 4B campaign — product/UI findings, deliberately not fixed in these test/verdict lanes):** finding-8 — frozen Manage Models `gr.Dataframe` on gradio 6.20.0 (one stale row, never re-renders despite correct `/models`+`/health` and a live 5 s self-heal Timer whose SSE flows; full DOM evidence in `docs/testing/step4a-e2e-unhollow-2026-09-12.tdd.md`, issue draft in the 4B evidence doc; 4A finding 2's intermittent dead-tabpanel is a related symptom; fix directions: gradio 6.20 Dataframe value-update path vs `wrap=True`/`interactive=False`/row-count diffing, or a non-Dataframe component) and finding-9 — MLX unload never returns memory to the OS (`mx.clear_cache()` missing in `unload_model_cleanup()`'s MLX branch; warm-reload-latency tradeoff to weigh before clearing unconditionally; issue draft with repro in the 4B evidence doc).
...
### Step 4B.2 — P1: engine correctness invariants + client contracts ✅ EXECUTED 2026-09-18

- **Model tier:** default · **Branch:** `test/p1-invariant-coverage` · **Source:** coverage
  analysis items 6–10.
- **Executed:** one commit per item, each proven non-hollow by mutation testing (mutate the
  source → the new test goes red → `git checkout --` restore → re-verify green; the mutation
  evidence is quoted in each commit body).

  | Item | Commit | Evidence |
  |------|--------|----------|
  | 7 — `_transcribe_probe` | `413da807` | 8 tests in `test_icl_echo_trim.py`; 2 mutants caught (the clip mutant showed unclipped samples wrap to the OPPOSITE SIGN, not merely saturate) |
  | 6 — torch streaming + parity | `84f739a2` | 11 tests in `test_engine_streaming.py`; the `_postprocess_chunk` parity invariant and the echo-trim-first-chunk-only rule each pinned by a mutant-killing test |
  | 8 — `client/generator.py` | `7b77be37` | 16 tests in `test_client_generator.py`; module 83% → **100%** (all 31 missed lines); 4 mutants, 1 initially SURVIVED (see below) |
  | 9 — vLLM circuit routing | `69fca2af` | 7 tests in `test_server_vllm_integration.py::TestVLLMCircuitRouting`; 2 mutants caught; no live vLLM needed |
  | 10 — UI torch create-prompt | `be58d4e9` | 8 tests in the new `test_ui_create_voice_prompt_torch.py` (registered in `BATCHES` batch 4); 4 mutants caught |

- **Finding (carried forward as a review rule):** item 8's dialogue non-200 arm initially passed
  under its own mutant — the error body has no `results`, so `_first_result` raised the *same*
  `GenerationError` a few lines later and an `assertRaises`-only test passed for the wrong
  reason. An error-path test must assert the *message*, not just the exception type, whenever a
  downstream guard can raise the same class. Same shape as item 9's log-only hazard: the
  fallback-disabled mutant still emitted the "falling back to torch/MLX" WARNING while silently
  degrading, so a log assertion would have been hollow there too.
- **Deviation:** item 10's "coordinate with Step 0G" note was stale at execution time — 0G had
  already landed and routes the MLX branch through the engine writer (covered against the server
  handler by `test_issue236_mlx_create_prompt.py`). Item 10 was therefore scoped to unit-level
  torch-branch coverage only, with no E2E duplication. The plan's cited line range for item 9
  (`app_generation.py` 485–521) was likewise stale — the vLLM block now lives at ~638–700.
- **Context:**
...
- **Decision made 2026-09-22 — relocate, config only.** The runtime config moves to `~/.config/qwen3-tts/config.json`, where the auth token already lives; the repo-root `config.json` stays tracked as the stock default CI reads. **Correction to this entry's original wording:** it claimed relocation would "also protect voice prompts and generation history." That was wrong — `voice_prompts/` is gitignored (`.gitignore:27`) with zero files ever committed, `.voice_server.{log,pid,lock}` are gitignored, and generation history already lives at `~/.voice_history.jsonl`. `config.json` was the **only** exposed artifact. Nothing but the config moves.
