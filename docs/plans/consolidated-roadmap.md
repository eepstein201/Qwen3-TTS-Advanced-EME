...
## Performance / Quality / Accuracy — 2026-07-30 research (PRF-1..6, PRF-8 ✅ shipped; PRF-7 partial; PRF-9/10 open)
...
**Status (2026-08-14):** PRF-1..6 and PRF-8 all merged 2026-08-09 (PRs #143–#149). PRF-7 is partial: `mlx-audio>=0.4.6` merged via dependabot (#129, `15d9a7d`); the 0.4.7 bump is open PR #164. PRF-9 remains validation-gated and PRF-10 unbuilt.
...
| ◐ **PRF-7** — partial (0.4.6 ✅ dependabot #129, `15d9a7d`; 0.4.7 open PR #164) | **Bump mlx-audio 0.4.5 → 0.4.6** — ICL cache (clone TTFT ~−300 ms), streaming leak fix (#852/v0.4.2), continuous batching (v0.4.3), ~13% RTF (v0.4.6). **0.4.6 merged; the 0.4.7 bump is pending review (#164).** | Speed | High | Low (dep bump) | `pyproject.toml:29`; **validate in MLX env** (dep knot — see `pyproject.toml:45`) |
...
| **PRF-9** | **Investigate raising MLX `max_new_tokens=2048` cap** — model natively supports 32,768 tokens (~40 min); chunking is a *backend* limit. Fewer chunks → fewer seams. **Validate-first:** 12 Hz long-form stability + M2 Pro memory before shipping. **Not shipped — validation-gated.** | Quality | High (structural) | Low to test / Med to ship | `core/engine/inference.py:185-186,318-319,324` (`_run_inference_mlx`), `server/client/generator.py:39` (`_generation_timeout`), `CLAUDE.md` |
