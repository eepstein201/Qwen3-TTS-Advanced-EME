<!-- Generated: 2026-09-21 | Files scanned: server/ (8.2k LOC), config/pm2.py (128 LOC) | Token estimate: ~890 -->
...
## Generation path
`/generate → cache check → on-demand load if the model slot is empty (#300: load_model_deduped, pre-lock) → run_inference (chunk + backend.generate + _postprocess_chunk) → {chunks, seed}`.
...
- **String-field bounds (6O #319 / 6P #320):** `_EMAIL_RE` quantifiers bounded (ReDoS event-loop stall on unauthenticated input) and all remaining `GenerateRequest` string fields length-bounded in `validation.py` — same boundary discipline as #263.
- **Activity watchdog (6H #318):** a single daemon watchdog thread in `app_lifespan.py` replaced the per-request activity `Timer`s — the old `reset_activity_timer` could strand a rescheduled timer on a lost cancel.
...
## Key files (LOC)
app_generation.py 1228 · app.py 1180 · app_lifespan.py 843 · websocket.py 654 · client/generator.py 620 · app_models.py 604 · app_prompts.py 561 · validation.py 549 (33 Pydantic models) · model_loading.py 541 · generation_state_guard.py 371 (0C #270) · vllm_client.py 332 · prompt_loading.py 73
