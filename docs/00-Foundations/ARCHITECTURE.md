...
On `/ws` the `HTTPException` surfaces as the terminal error frame (+ RFC 6455 close 1011) rather than an HTTP status — coherent retryable-abort semantics on a socket. Accepted costs: `unload_model_cleanup`'s gc + `gen_cache` wipe now run under the lock (bounded, same shape as `/unload-asr`). Guarded by `tests/test_issue214_unload_queued_window.py` (behavioral + AST-enclosure pins; 5 mutants killed) and the live choreography in `tests/test_e2e_queueing.py::test_04`. Residual (review-enforced, not test-enforced): a deliberately restructured double-acquire implementation could reopen a window between two acquisitions — rejected by review; the enclosure pins require the re-read inside a locked block and the behavioral tests pin acquire→raise/inference ordering.
