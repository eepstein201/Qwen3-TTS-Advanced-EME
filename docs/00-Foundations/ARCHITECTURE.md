...
- Token: `~/.config/qwen3-tts/.voice_server_token` (0o600 perms; legacy fallback `~/.voice_server_token`), preserved on normal shutdown so clients keep authenticating across restarts; removed by `POST /shutdown` and by the startup lock when it detects a stale mismatched token
...
- Gradio UI surface: every launch site (`ui/_facade.py` `main()` and `generate_server.build_ui_and_launch`) routes launch kwargs through `get_gradio_launch_kwargs(config, share=...)` (`ui/shared.py`), which enforces share ⇒ auth: a shared (public) URL always launches behind a login — `TTS_UI_USERNAME`+`TTS_UI_PASSWORD` when BOTH are set, else generated credentials delivered hybrid: the `token_urlsafe` password is written as the sole line of a 0600 file (`~/.config/qwen3-tts/.ui_share_credentials`, the server-token dir; truncated on each generated launch) and only the username + file path are printed once, so the password string never reaches any log sink; a half-set env pair, a credential-generation failure, or a credentials-file write failure raises RuntimeError instead of launching unauthenticated (fail-closed). `IN_COLAB` forces share at both sites, so the Colab path is authenticated too (the notebook cell mirrors the helper inline). Custody rationale: the UI process holds the server bearer token, so an unauthenticated shared URL would hand out the full authenticated server surface — generation, prompt delete/rename, `/update-model-config`, `/shutdown`, and hard-delete of output files. `allowed_paths` is narrowed to the `history_output_directory` root (default `~/Downloads/Qwen3-TTS Output`, via `resolve_history_output_dir`) + the system tempdir — the former blanket `~/Downloads` entry is gone, and the legacy `output_directory` resolver (`_resolve_output_dir`, whose default IS `~/Downloads`) would have made the narrowing a no-op under default config, so the helper resolves the history root instead
...
### `/load-model` in-flight dedup (#214 item 3)

Duplicate POSTs during a cold load used to build the weights twice (~2.5 GB double allocation): `handle_load_model` guarded only on *already loaded*, and the write-only `models_loading` flag was never a gate. Phase 2c replaces the flag with a per-load record table (`state.model_loads`, owned by `qwen3_tts/server/model_loading.py`): `claim_model_load` CAS-es the slot under a module-scope `MODEL_LOAD_LOCK` (fail-closed — the gate never degrades to absent), a duplicate gets `ATTACH` and awaits the owner's `done` Event (`MODEL_LOAD_WAIT_TIMEOUT_SEC=870 < LOAD_MODEL_TIMEOUT_SEC=900`, retryable 503 `load_in_progress` on timeout), and `release_model_load` runs in the owner's `finally` (identity-checked slot clear; **first terminal writer wins** — a superseder's CANCELLED is never stomped by a stale owner's late FAILED). Waiters classify from the RECORD, never shared dicts: FAILED with a message → 500 carrying it (mirroring the owner's `code`/`recovery`, so an `import_error` doesn't reach waiters as a retryable `load_failed`); FAILED with an empty message → retryable 503 (C3); CANCELLED → retryable 503. `state.model_config_epoch` is bumped by `/update-model-config` and `/unload-model` **under `MODEL_LOAD_LOCK`** — a caller whose epoch differs from the in-flight record does not attach, AND the owner re-checks `_is_superseded` (record CANCELLED **or** epoch mismatch) at its assign and error-write points, because supersede is LAZY: it fires only at the next claim, so an owner finishing after a bump with no intervening claimant would otherwise install old-config weights — it instead discards them and answers **503 `load_in_progress`**, never 200 (C1, both halves); `/unload-model` during an in-flight load is a structured 409 (H4a). W1: a design warm-up failure keeps the model (startup precedent) — `warmup_failed: true` on owner AND waiter responses, never written to `model_load_errors` (its only non-display read sits behind `models.get(mode) is None`, and /health lists non-None entries — recording it would report a usable model as broken), and no warm-up retry (first request pays the cold start). `models_loading` is DELETED; `/models` derives its `loading` display flag from the record table (wire field unchanged). `_background_load` WAITS on an HTTP-owned record instead of skipping (H2 — skipping would set `models_loaded`, which gates `/ready`, while the HTTP-owned load is minutes away; an owner failure is recorded in `model_load_errors` so `/health` surfaces it), skips any model already loaded (a concurrent HTTP load wins), and deliberately does NOT run `_recover_from_failed_load` (pre-existing asymmetry — tracked follow-up), which moved to `model_loading.py` with the owner body. Ops residual (deliberate, H2): an HTTP-triggered cold load during startup keeps `/ready` at 503 for up to ~14.5 min (870 s wait) — a health check that restarts on prolonged 503 will kill a healthy loading server. Single-process assumption (like `inference_lock`). Guarded by `tests/test_issue214_load_model_dedup.py` (12-mutant matrix + 2 review-fix mutants, all killed) + `tests/test_response_contracts.py` dedup/warmup round-trips. #214 item 4 (the e2e queuing tier) ships as `tests/test_e2e_queueing.py`; the unload-queued-generation window (T5, a 2c split-out that this clause previously mis-numbered as item 4 and mis-diagnosed as "needs a mode-aware claim") is closed by the leaf lock — see below.
...
The unload-asr race is CLOSED: `unload_asr_model` takes `_asr_lock` (it was the module's only unsynchronized writer) AND `/unload-asr` acquires `inference_lock` — `_asr_lock` alone merely narrowed the window, since an unload could still land between `/transcribe`'s post-lock recheck and `transcribe_audio`'s own lazy-load check (`asr.py:168`, or `_transcribe_torch`'s unconditional `_ensure_asr_torch_loaded`) and rebuild the model INSIDE the serialized section. Locking the unload closes it structurally, and closes the identical check-then-use on the ICL echo-trim probe (`inference.py:1165`→`:1173`) for free, since that runs with `inference_lock` already held. `/transcribe` re-checks under the lock and returns a retryable 503 `asr_unloaded` rather than reloading; every `/unload-asr` client must use `UNLOAD_ASR_TIMEOUT_SEC` (=900, `core/http_client.py`) since the unload now queues behind a generation. #193/Step 1C reshaped the echo-trim half around this same principle: the server now ensure-loads ASR UNLOCKED — via `asyncio.to_thread` in `_ensure_asr_for_echo_trim` (`app_generation.py`), BEFORE a clone generation queues for `inference_lock`, gated on the probe's own three conditions (`trim_icl_echo` true + `mode=clone` + a transcript resolvable via `_reference_text_from_prompt`) — and KEEPS it loaded (mirrors `/transcribe`'s unlocked-load split; a cold HF download must never run in-lock). The in-lock `is_asr_loaded()` check is now the documented DEGRADATION path, not an opportunistic gate: a miss means an `/unload-asr` landed in the unlocked preload window, and the trim is skipped — untrimmed output, never a failed generation and never an in-lock rebuild (unlike `/transcribe`'s 503: a missing echo-trim is cosmetic). The 50% cut cap scales to the FIRST chunk on the multi-chunk batch path (`trim_cap_samples`, WS9.4).
...
### `/unload-model` leaf lock (T5)

An unload landing while a generation is queued used to produce a **lying 200**: every generation path captures `state.models[mode]` into a local BEFORE acquiring `inference_lock` and sets `generation_state["active"]` only AFTER, so the `active`+mode 409 could not see the queued request — the unload nulled the slot, bumped the epoch, wiped `gen_cache`, and answered `unloaded` while the queued generation ran to completion on the orphaned local (the engine never re-reads app state; `unload_model_cleanup` is gc + accelerator-cache only, never destroys weights). A later `/load-model` seeing the empty slot then **double-allocated** — the #233 pathology re-opened through a new route. The fix is four coordinated pieces (the route lock alone is self-defeating — see the second):

1. **Route leaf lock** (`app.py` `unload_model`): mirrors `/unload-asr`. FIFO orders acquisitions, not arrivals; a generation still in its pre-lock section can be overtaken (it then gets the re-read's retryable 503, never an orphan run); the lock is global, so unloading an idle model waits behind ANY model's generation (a cross-model stall on `already_unloaded` is accepted); a mid-batch unload 409s fast and honestly while the batch is genuinely active. Clients use `UNLOAD_MODEL_TIMEOUT_SEC` (=900, `core/http_client.py`, drift-guarded) — the old hardcoded 10s failed spuriously whenever anything was queued; `/update-model-config`'s client uses it too.
2. **In-lock `generation_state` reset on the FINAL batch item** (`app_generation.py`): the batch tail (encode / cache-write / peaks, each an off-loop await) releases the lock long before the outer `finally` clears `active`, so with the route lock alone a queued unload acquires with stale `active=True` and 409s AFTER the full wait. Streaming and `/ws` already reset in-lock. The outer `finally` remains as an idempotent safety net.
3. **Post-lock slot re-read** (`_require_model_under_lock`, `app_generation.py`): batch (per item), streaming, and `/ws` re-read `state.models.get(mode)` under the lock; `None` → retryable 503 `model_unloaded` (canonical shape: `/transcribe`'s `asr_unloaded` recheck). This closes the capture→acquire gap: an unload that overtakes a not-yet-parked generation turns into a clean retry for that generation instead of an orphan run.
4. **`/update-model-config` takes the same route lock** — it nulls ALL THREE slots with no active guard at all (strictly weaker than pre-fix `/unload-model`); `save_config` runs in a worker thread while the lock is held.

Error surfacing differs per transport (Gate B): on `/generate-stream` the response headers are already committed when the generator iterates, so the re-read's 503 goes out as the **in-band terminal error frame** (the WS2 `sr==0` sentinel) — raising there would truncate the connection with no terminal frame, indistinguishable from a network drop. On `/ws` the re-read is caught and sent as a clean `{"error": <message>}` message (the validation handler's shape); letting the `HTTPException` escape would stringify it in the generic `except` and mangle the payload. Accepted costs: `unload_model_cleanup`'s gc + `gen_cache` wipe now run under the lock (bounded, same shape as `/unload-asr`). Guarded by `tests/test_issue214_unload_queued_window.py` (behavioral + AST-enclosure pins; 5 mutants killed) and the live choreography in `tests/test_e2e_queueing.py::test_04`. Residual (review-enforced, not test-enforced): a deliberately restructured double-acquire implementation could reopen a window between two acquisitions — rejected by review; the enclosure pins require the re-read inside a locked block and the behavioral tests pin acquire→raise/inference ordering.

### Cap-warning field watch (#214 item 5)

The #192 cap detector emits `logger.warning` on logger `tts.engine` from exactly two MLX-only call sites — batch (`inference.py:568`) and streaming (`:729`, per-segment delta accumulation); **torch never emits it**, so a torch-side runaway is invisible to this watch. The greppable string is the message text, NOT a function name (`_warn_if_cap_reached` greps to nothing by construction — #214's item-5 body originally told the reader to grep for the function name). Watch with:

```bash
grep -n "token cap without emitting EOS" .voice_server.log .voice_server.log.1
```

(include `.voice_server.log.1` — the 5 MB rotation with one backup means a hit can fall out of the live file). For an active probe, `scripts/probe_issue192.py` already counts these lines and exits 1 on any hit. Baseline: the only hit in the current log is dated 2026-08-24, before the #192 pairs closed — consistent with the closed model, not a counterexample. Any hit now is a **new lead**, not a recurrence of the closed pairs.
