...
| `generation.trim_icl_echo` | boolean | `true` | Clone-only: clip the ICL echo of the reference transcript (upstream #341) from the head of cloned output. When a clone generation would use it (mode=clone, a reference transcript resolvable, ASR not already loaded), the server **force-loads ASR** — unlocked, before the generation queues for `inference_lock` — and **keeps it loaded**. If ASR is absent under the lock (`/unload-asr` landed in the preload window), the generation ships **untrimmed** rather than failing. The 50%-of-output trim cap scales to the **first chunk** on multi-chunk batch output; `x_vector_only_mode` prompts carry no transcript and are never probed. |
...
User-defined presets can be saved and deleted directly from the web UI (Clone tab → "My generation presets"): saves capture the current Temperature / Top-K / Top-P / Repetition Penalty sliders under a name (letters, numbers, spaces, dashes, underscores, dots; max 40 chars — spaces are allowed here but not in prosody preset names), overwrites and deletes use a 5-second click-again confirm, and the 8 factory names are reserved on both paths. A save or delete refreshes the Preset dropdown on all three tabs immediately; a save never changes any tab's current selection, and a delete resets only the dropdowns that were showing the deleted preset back to `(none)`. Delete removes any non-factory name, including one hand-edited into `config.json`; a hand-edited factory-name override still applies via the merged view but is not manageable in the UI — restore the factory values by editing `config.json`.
