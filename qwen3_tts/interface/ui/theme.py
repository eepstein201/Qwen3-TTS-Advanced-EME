...
UI_CSS = (
    ...
    + ".ws-btn{min-width:40px;min-height:40px;padding:0 14px;"
    f"border-radius:{var('radius_md')};border:1px solid {var('border_strong')};"
    "background:var(--button-secondary-background-fill,#e5e7eb);"
    "color:var(--button-secondary-text-color,#374151);cursor:pointer;font-size:0.9em;"
    "transition-property:transform,background-color;"
    f"transition-duration:{var('dur_fast')};}}"
    ".ws-btn:active{transform:scale(0.96);}"
    f".ws-btn:focus-visible{{outline:2px solid {var('focus')};outline-offset:2px;}}"
    ".ws-btn:disabled{opacity:0.5;cursor:not-allowed;}"
    f".ws-waveform{{width:100%;min-height:80px;background:{var('surface')};"
    f"border-radius:{var('radius_md')};margin-bottom:8px;overflow:hidden;}}"
    ".ws-controls{display:flex;align-items:center;gap:8px;}"
    + _severity_rules()
