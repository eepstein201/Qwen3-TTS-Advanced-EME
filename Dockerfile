...
# Create expected directory structure (config at ~/.config/qwen3-tts/, voice
# prompts under ~/Qwen3-TTS_UserFiles/)
RUN mkdir -p /root/Qwen3-TTS_UserFiles /root/.config/qwen3-tts /app/voice_prompts && \
    ln -sf /app/config.json /root/.config/qwen3-tts/config.json && \
