...
class TestVoicePromptExists(unittest.TestCase):
    ...
    def test_mlx_backend_strips_wav_suffix(self, mock_exists, _mock_backend):
        ...
        get_default_clone_prompt() returns the stored filename ("sample_voice.wav")
        ...
        voice_prompts/sample_voice.wav.wav (bare `tts start` repro, 2026-09-11).
        """
        from qwen3_tts.interface.generate_helpers import voice_prompt_exists
        probed = []
        mock_exists.side_effect = lambda p: (probed.append(p), True)[1]
        self.assertTrue(voice_prompt_exists("sample_voice.wav"))
        ...
        self.assertTrue(wav_probes[0].endswith("/sample_voice.wav"))
