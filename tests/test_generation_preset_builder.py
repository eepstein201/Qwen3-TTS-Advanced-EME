...
# for a hand-edited non-factory key that must survive raw writes untouched.
...
class TestSaveUserGenerationPreset(unittest.TestCase):
    ...
    def test_save_new_preserves_raw_base(self):
        ...
        self.assertIn("junk-entry", written["presets"])
...
class TestSaveHandler(unittest.TestCase):
    ...
    def _call(self, state, name, existing=None, writer=None, choices=None):
    ...
    def test_new_preset_saves_and_refreshes_all_three_dropdowns(self):
        ...
        self.assertIn("Saved preset 'mine'", result[2])
        self.assertEqual(result[0], DISARMED)
        for slot in (4, 5, 6):
            self.assertEqual(result[slot], gr.update(choices=["(none)", "mine"]), slot)
        self.assertEqual(result[7], gr.update(choices=["(none)"]))
...
class TestDeleteHandler(unittest.TestCase):
    ...
    def _call(
        ...
        with (
            ...
            mock.patch(
                "qwen3_tts.interface.ui.shared.get_user_generation_preset_choices",
                return_value=["(none)"]
                + sorted({k for k in (existing or {}) if k != selection}),
            ),
        ):
            result = self._handler()(
    ...
    def test_delete_arms_on_first_click(self):
        ...
        self.assertIn("Click Delete again", result[2])
    ...
    def test_fresh_arm_confirms_and_conditionally_resets(self):
        ...
        self.assertIn("Deleted preset 'mine'", result[2])
        self.assertEqual(result[0], DISARMED)
        fresh = ["(none)", "other"]
...
class TestGenerationPresetUiWiring(unittest.TestCase):
    ...
    def test_reuses_generic_prosody_mechanics(self):
        src = self._tabs_source()
        self.assertIn("_prosody_arm_is_fresh(", src)
        self.assertIn("_PROSODY_DISARMED_STATE", src)
    ...
    def test_facade_wires_clicks_with_builder_refs(self):
        src = self._facade_source()
        # Format-agnostic windows around each wiring call.
        for needle in (
            "_on_save_generation_preset",
            "_on_delete_generation_preset",
        ):
            ...
            self.assertIn("preset_builder", window, needle)
