...
class TestTTSConfig(unittest.TestCase):
    ...
    def test_read_auth_token_missing(self):
        ...
        with patch(
            "qwen3_tts.core.config.TOKEN_FILE",
            os.path.join(tempfile.gettempdir(), "nonexistent_token_test_xyz"),
        ):
...
class TestBackendConfig(unittest.TestCase):
    ...
    def test_get_backend_env_override(self):
        ...
        with patch.dict(os.environ, {"TTS_BACKEND": "mlx"}):
            with patch(
                "qwen3_tts.core.config.load_config",
                return_value={"advanced": {"backend": "torch"}},
            ):
                result = get_backend()
        self.assertEqual(result, "mlx")
    ...
    def test_get_backend_invalid_falls_back(self):
        ...
        with patch.dict(os.environ, {}, clear=False):
            ...
            with patch(
                "qwen3_tts.core.config.load_config",
                return_value={"advanced": {"backend": "invalid"}},
            ):
                result = get_backend()
        self.assertEqual(result, "mlx")
    ...
    def test_get_mlx_quantization_from_config(self):
        ...
        with patch(
            "qwen3_tts.core.config.load_config",
            return_value={"advanced": {"mlx_quantization": "4bit"}},
        ):
            result = get_mlx_quantization()
    ...
    def test_get_mlx_quantization_invalid_falls_back(self):
        ...
        with patch(
            "qwen3_tts.core.config.load_config",
            return_value={"advanced": {"mlx_quantization": "garbage"}},
        ):
            result = get_mlx_quantization()
        self.assertEqual(result, "8bit")
...
class TestLazyImports(unittest.TestCase):
    ...
    def test_engine_no_torch_at_module_scope(self):
        ...
        for mod in list(sys.modules.keys()):
            if mod == "qwen3_tts.core.engine" or mod.startswith(
                "qwen3_tts.core.engine."
            ):
...
class TestPlatformDetection(unittest.TestCase):
    ...
    def test_get_device_returns_cuda_with_env(self):
        ...
        with (
            patch("qwen3_tts.core.config.IS_MACOS", False),
...
class TestAuthTokenReaders(unittest.TestCase):
    """4B.3 item 13: core/config/auth.py legacy-path fallback and the
    empty-header arm (previously missed lines 29-38, 49).

    auth.py resolves TOKEN_FILE / _LEGACY_TOKEN_FILE through the package
    facade on every call, so patching the facade attributes is the seam.
    """

    def _write_token(self, content):
    ...
    def _missing_path(self):
        return os.path.join(tempfile.gettempdir(), "no_such_token_file_xyz")

    def test_reads_primary_token_without_warning(self):
        from qwen3_tts.core.config import read_auth_token

        primary = self._write_token("primary_tok\n")
        with (
            patch("qwen3_tts.core.config.TOKEN_FILE", primary),
            patch("qwen3_tts.core.config._LEGACY_TOKEN_FILE", self._missing_path()),
            self.assertNoLogs("tts", level="WARNING"),
        ):
            self.assertEqual(read_auth_token(), "primary_tok")

    def test_legacy_fallback_reads_token_and_warns(self):
        from qwen3_tts.core.config import read_auth_token

        legacy = self._write_token("legacy_tok")
        with (
            patch("qwen3_tts.core.config.TOKEN_FILE", self._missing_path()),
            patch("qwen3_tts.core.config._LEGACY_TOKEN_FILE", legacy),
            self.assertLogs("tts", level="WARNING") as logs,
        ):
            self.assertEqual(read_auth_token(), "legacy_tok")
        self.assertTrue(any("legacy path" in line for line in logs.output), logs.output)
    ...
    def test_no_token_anywhere_returns_none(self):
        ...
        with (
            ...
            self.assertIsNone(read_auth_token())

    def test_auth_headers_bearer_when_token_present(self):
        from qwen3_tts.core.config import auth_headers

        primary = self._write_token("hdr_tok")
        with (
            patch("qwen3_tts.core.config.TOKEN_FILE", primary),
            patch("qwen3_tts.core.config._LEGACY_TOKEN_FILE", self._missing_path()),
        ):
            self.assertEqual(auth_headers(), {"Authorization": "Bearer hdr_tok"})

    def test_auth_headers_empty_when_token_missing(self):
        from qwen3_tts.core.config import auth_headers
...
class TestConfigReaderFallbacks(unittest.TestCase):
    """4B.3 item 13: exception/invalid fallback arms of core/config/models.py
    readers (previously missed 52-55, 82-83, 101-102, 127-130, 173-182,
    212-216, 368, 420-427).

    Every reader imports load_config lazily from the package facade, so the
    patch target is the facade attribute.
    """

    def test_broken_config_file_yields_defaults(self):
        from qwen3_tts.core.config import (
            get_backend,
        ...
        # Reference default for get_backend (platform-dependent) captured
        # from an empty-but-valid config first.
        with patch("qwen3_tts.core.config.load_config", return_value={}):
            expected_backend = get_backend()

        with patch(
            "qwen3_tts.core.config.load_config", side_effect=ValueError("bad json")
        ):
            self.assertEqual(get_torch_dtype_name(), "float32")
            self.assertEqual(get_backend(), expected_backend)
            self.assertEqual(get_mlx_quantization(), "8bit")
            self.assertEqual(get_model_size(), "1.7B")
            self.assertEqual(get_torch_quantization(), "none")

    def test_invalid_values_yield_defaults(self):
        from qwen3_tts.core.config import (
            get_backend,
            get_mlx_quantization,
            get_model_size,
    ...
    def test_get_torch_quantization_valid_value_passes_through(self):
        from qwen3_tts.core.config import get_torch_quantization

        with patch(
            "qwen3_tts.core.config.load_config",
            return_value={"advanced": {"torch_quantization": "8bit"}},
        ):
            self.assertEqual(get_torch_quantization(), "8bit")

    def test_get_torch_quantization_invalid_value_defaults(self):
        from qwen3_tts.core.config import get_torch_quantization

        with patch(
            "qwen3_tts.core.config.load_config",
            return_value={"advanced": {"torch_quantization": "13bit"}},
        ):
            self.assertEqual(get_torch_quantization(), "none")

    def test_get_config_value_arms(self):
        from qwen3_tts.core.config.models import _get_config_value

        with patch(
            "qwen3_tts.core.config.load_config",
            return_value={"cache": {"voice_prompt_max": 7, "none_key": None}},
        ):
            self.assertEqual(_get_config_value(["cache", "voice_prompt_max"], 5), 7)
            # Missing key falls back to the default.
            self.assertEqual(_get_config_value(["cache", "absent"], 5), 5)
            # Failing validator falls back to the default.
            self.assertEqual(
                _get_config_value(
                    ["cache", "voice_prompt_max"], 5, validator=lambda v: v < 5
                ),
                5,
            )
            # None value falls back to the default.
            self.assertEqual(_get_config_value(["cache", "none_key"], 5), 5)

        with patch(
            "qwen3_tts.core.config.load_config", side_effect=OSError("unreadable")
        ):
            self.assertEqual(_get_config_value(["cache", "voice_prompt_max"], 5), 5)

    def test_get_mlx_model_name_unknown_size_raises(self):
        from qwen3_tts.core.config import get_mlx_model_name

        with patch("qwen3_tts.core.config.get_model_size", return_value="9B"):
            with self.assertRaisesRegex(ValueError, "Unknown model size"):
                get_mlx_model_name("clone")

    def test_get_model_revision_pinned_config_wins(self):
        from qwen3_tts.core.config import get_model_revision

        with patch(
            "qwen3_tts.core.config.load_config",
            return_value={"models": {"clone": {"revision": "sha123"}}},
        ):
            self.assertEqual(get_model_revision("clone"), "sha123")

    def test_get_model_revision_falls_back_to_model_info(self):
        from qwen3_tts.core.config import get_model_revision
        ...
        with (
            ...
            patch(
                ...
                return_value={"revision": "tag9"},
            ),
        ):
            self.assertEqual(get_model_revision("clone"), "tag9")

    def test_get_model_revision_defaults_to_main(self):
        from qwen3_tts.core.config import get_model_revision

        with (
            patch("qwen3_tts.core.config.load_config", return_value={}),
            patch("qwen3_tts.core.config.get_model_info", return_value={}),
        ):
            self.assertEqual(get_model_revision("clone"), "main")
